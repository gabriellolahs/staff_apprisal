...
$staff_id = $_SESSION['staff']; // ✅ correct way
...
if (!$staff) {
    echo "<p style='color:red;'>No staff record found for ID: " . htmlspecialchars($staff_id) . "</p>";
    exit();
}
...
if (!empty($staff['passport'])) {
    if (file_exists($staff['passport'])) {
        $passport = $staff['passport'];
    } elseif (file_exists("uploads/" . $staff['passport'])) {
        $passport = "uploads/" . $staff['passport'];
    }
}

// Fetch biodata + school/department names
$sql = "
...
if ($result->num_rows > 0) {
    ...
    $is_locked = (int)($biodata['is_locked'] ?? 0);
} else {
    $biodata = [];
}
...
$selectedSchoolId = $biodata['school'] ?? $staff['school_id'] ?? null;
$selectedDeptId = $biodata['department'] ?? $staff['department_id'] ?? null;
