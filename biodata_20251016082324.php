...
// Fetch biodata (join school + department)
$sql = "
...
if ($result->num_rows > 0) {
    ...
    $is_locked = isset($biodata['locked']) ? $biodata['locked'] : 0;
} else {
    $biodata = [];
}
...
// Fetch all schools
$schools = [];
$res = $conn->query("SELECT id, school_name FROM schools ORDER BY school_name ASC");
...
$departments = [];
$res2 = $conn->query("SELECT id, department_name FROM departments ORDER BY department_name ASC");
  ...
  <?php endif; ?>
    ...
    <?php foreach ($schools as $school): ?>
        <option value="<?php echo $school['id']; ?>"
            <?php echo (isset($biodata['school']) && $biodata['school'] == $school['id']) ? 'selected' : ''; ?>>
    ...
    <?php endforeach; ?>
      ...
      <div class="col-md-3">
        <select name="department" class="form-control" <?php echo $disabled; ?>>
    <option value="">-- Select Department --</option>
    <?php foreach ($departments as $dept): ?>
        <option value="<?php echo $dept['id']; ?>"
            <?php echo (isset($biodata['department']) && $biodata['department'] == $dept['id']) ? 'selected' : ''; ?>>
