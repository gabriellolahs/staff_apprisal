...
// Fetch biodata with school and department names
$sql = "
...
$schools = $conn->query("SELECT * FROM schools ORDER BY school_name ASC")->fetch_all(MYSQLI_ASSOC);
$departments = $conn->query("SELECT * FROM departments ORDER BY department_name ASC")->fetch_all(MYSQLI_ASSOC);

// Determine selected school/department
$selectedSchoolId = $biodata['school'] ?? $staff['school_id'] ?? $staff['school'] ?? null;
$selectedDeptId   = $biodata['department'] ?? $staff['department_id'] ?? $staff['department'] ?? null;

if (!empty($biodata['is_locked']) && $biodata['is_locked'] == 1) {
    $readonly = 'readonly';
    $disabled = 'disabled';
} else {
    $readonly = '';
                ...
                <select class="form-control" id="school" name="school" <?php echo $disabled; ?>>
                    ...
                    <?php foreach ($schools as $school): ?>
                        <option value="<?php echo $school['id']; ?>"
                            <?php echo ($selectedSchoolId == $school['id']) ? 'selected' : ''; ?> required>
                ...
                <select class="form-control" id="department" name="department" <?php echo $disabled; ?>>
                    ...
                    <?php foreach ($departments as $dept): ?>
                        <option value="<?php echo $dept['id']; ?>"
                            <?php echo ($selectedDeptId == $dept['id']) ? 'selected' : ''; ?> required>
                            <?php echo htmlspecialchars($biodata['title'] ?? ''); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
