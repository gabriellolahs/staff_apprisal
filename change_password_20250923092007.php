...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($staff && password_verify($current, $staff['password'])) {
        if ($new === $confirm && !empty($new)) {
            ...
            $update = $conn->prepare("UPDATE staff SET password = ?, has_password = 1 WHERE staff_id = ?");
            $update->bind_param("ss", $hashed, $staff_id);
            if ($update->execute()) {
                echo "<script>alert('Password updated successfully! You must log in with your Staff ID and password.'); window.location='academic_dashboard.php';</script>";
            } else {
        ...
        <div class="details">
            ...
            <div class="card p-4 bg-white">
                <form method="POST">
                    ...
                    <a href="academic_dashboard.php" class="btn-back">Back to Dashboard</a>
                </form>
            </div>
        </div>
    </div>
