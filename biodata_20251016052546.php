...
// ✅ Check if biodata already locked
$bio_stmt = $conn->prepare("SELECT * FROM staff_biodata WHERE staff_id = ?");
$bio_stmt->bind_param("s", $staff_id);
$bio_stmt->execute();
$bio_result = $bio_stmt->get_result();
$biodata = $bio_result->fetch_assoc();

$is_locked = ($biodata && $biodata['locked'] == 1);
  ...
  <?php endif; ?>
    ...
    <div class="row mb-3">
      ...
      div class="col-md-3">
        ...
        <input type="text" class="form-control" name="title" <?php echo $readonly; ?>required></div>
    </div>
